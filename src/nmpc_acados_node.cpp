...
{ 
	...
	{ 
		path = msg; 
		receive_traj = true; 
		printf("Received new trajectory with %d poses.", int(msg.poses.size()));
	} 
	else 
	{
        printf("New trajectory received but already have one.");
    }
}
...
{
    ros::init(argc, argv, "nmpc_acados_node");
    ros::NodeHandle nh("~");

	ros::Subscriber state_sub = nh.subscribe("/odometry/filtered", 10, odomCallback);
    ros::Subscriber path_sub = nh.subscribe("/trajectory", 1, pathCallback);

    ros::Publisher vel_pub = nh.advertise<geometry_msgs::Twist>("/cmd_vel", 10);
    ros::Publisher predict_pub = nh.advertise<nav_msgs::Path>("/predicted_path", 10);
    ...
    while(ros::ok())
    {
		...
		// set_x0 
		ocp_nlp_constraints_model_set(nlp_config, nlp_dims, nlp_in, nlp_out, 0, "lbx", acados_x0);
		ocp_nlp_constraints_model_set(nlp_config, nlp_dims, nlp_in, nlp_out, 0, "ubx", acados_x0);
		...
		for (int i = 0; i <= N; i++)
		{
			...
			ref_pose.pose.orientation = createQuaternionMsgFromYaw(ref_states[i * NX + 2]);
		...
		std::cout << "Control output: linear = " << control_output[0][0] 
        			<< ", angular = " << control_output[0][1] << std::endl;
